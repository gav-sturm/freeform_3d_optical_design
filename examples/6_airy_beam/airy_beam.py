...
def get_airy_training_pair(coords, wavelength):
    """
    Generates a physically accurate, energy-conserved training pair for
    creating an Airy beam generator.
    """
    ...
    # --- START OF NEW AIRY BEAM CODE ---
    # 2. Define the Desired Output Field: An Airy beam
    
    # Airy beam parameters
    alpha = 0.1  # Controls the strength of the "bend"
    
    # The amplitude is a Gaussian envelope (a realistic target)
    output_beam_waist = 4.0
    radius_sq = x**2 + y**2
    desired_amplitude = np.exp(-radius_sq / (2 * output_beam_waist**2))
    
    # The phase is a cubic function, which causes the acceleration
    cubic_phase = alpha * (x**3 + y**3)
    
    desired_output_field = (desired_amplitude * np.exp(1j * cubic_phase)).astype('complex128').squeeze()
    # --- END OF NEW AIRY BEAM CODE ---
...
def airy_simulation():
    ...
    for iteration in range(int(1e6)): # Run for a loooong time
        ...
        ### NEW CODE FOR Airy Beam ###
        input_field, desired_output_field = get_airy_training_pair(coords, wavelength)
