...
stage. The grid sizes are chosen to have large, power-of-two jumps
(e.g., 128 -> 256 -> 512) for straightforward interpolation.
...
def generate_training_schedule(
    ...
    grid sizes that follow large, power-of-two jumps.
    """
    ...
    # --- MODIFIED LOGIC: Grid sizes jump by powers of two ---
    # Generate grid sizes that are powers of 2, from start_n_xy to end_n_xy.
    # This creates distinct, large jumps like 128 -> 256 -> 512 -> 1024 -> 2048.
...
# --- Generate a 5-stage training schedule with power-of-two jumps ---
# TRAINING_STAGES = generate_training_schedule(
#     num_stages=5,          # Reduced stages for big jumps
...
# --- Run a single 512x512 stage from scratch ---
TRAINING_STAGES = [
    {
        'name': 'Stage_512_Single_Test',
        'physical_width_um': 159.4, # Corresponds to the 3rd stage in the original schedule
        'n_xy': 512,
        'n_z': 512,
        'iterations': 1000,
        'step_size': 100,
        'smoothing_sigma': 3.5, # Corresponds to the 3rd stage
    }
]
